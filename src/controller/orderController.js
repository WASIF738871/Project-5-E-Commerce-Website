...
const createOrder =  async(req,res)=>{
        try{
            ...
            if(cartExist.items.length==0){
                return res.status(404).send({status:false,msg:"There Is No Items for Order In Ur Cart"})
            }
            if(cartExist){
                    ...
                    for(let i=0; i<cartExist.items.length; i++){
                        let obj = {
                            productId:cartExist.items[i].productId,
                            quantity:cartExist.items[i].quantity
                        }
                        data['items'] = obj
                ...
                return res.status(201).send({status:true,msg:"Order Created SuccessFull",Order:orderCreated})
...
const updateOrder = async(req,res) =>{
    ...
    try{
        ...
        if(!orderId){
            return res.status(400).send({status:false,msg:"Provide The OrderId"})
        }
        ...
        if(!orderExist){
            return res.status(404).send({status:false,msg:"There Is no order Exist"})
        }
        if(orderExist.userId!=userId){
            return res.status(403).send({status:false,msg:"Either this OrderId or userId is not Urs"})
        }
        if(orderExist.cancellable=='false'){
            return res.status(400).send({status:false,msg:"This Order Can't be cancelled"})
        }else{
            ...
            return res.status(200).send({status:false,msg:"order Updated",Order:updated})
        }
