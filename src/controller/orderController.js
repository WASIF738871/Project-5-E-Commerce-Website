    ...
    try {
        ...
        if (cartExist) {
            ...
            if (orderCreated) {
                ...
                await cartModel.findByIdAndUpdate({ _id: cartId }, { $set: cartExist });
            }
...
const updateOrder = async (req, res) => {
    try {
        ...
        let orderExist = await orderModel.findById({ _id: orderId, isDeleted: false });
        if (!orderExist) {
        ...
        if (!flag) {
            return res.status(400).send({ status: false, data: "Enter a valid Status: completed or canceled" });
        }
        ...
        return res.status(500).send({ status: flase, message: err.message });
    }
};
