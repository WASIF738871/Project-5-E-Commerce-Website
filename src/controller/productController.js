...
const updateProduct = async function (req, res) {
    try {
        ...
        //body empty
        if (!data || Object.keys(data).length === 0) return res.status(400).send({ status: false, message: "Provide Data in Body" })
        ...
        if (availableSizes) {
            ...
            for (const element of subtrim) {
                ...
                if (Size.includes(element) === false) return res.status(400).send({ status: false, message: 'Sizes should be in ["S", "XS", "M", "X", "L", "XXL", "XL"]' })
