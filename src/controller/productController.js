...
const getProductByFilter = async function (req, res) {
    try {
        ...
        let { size, name, priceGreaterThan, priceLessThan, priceSort } = filter;
        ...
        if (priceSort != null) {
            if (priceSort.length > 0) {
                if ((priceSort == 1 || priceSort == -1)) {
                    ...
                    return res.status(400).send({ status: false, message: "priceSort key should have only 1 or -1 " });
                }
            } else {
                return res.status(400).send({ status: false, message: "priceSort key should hav value -1 or 1 " });
            }
        ...
        const getData = await productModel.find(data).sort(sort);
        if (getData.length == 0) {
...
const updateProduct = async function (req, res) {
    try {
        ...
        if (availableSizes != null) {
            ...
            let subtrim = size.map((element) => {
                return element.trim();
            });

            for (let element of subtrim) {
                if (Size.includes(element) === false)
                    return res.status(400).send({ status: false, message: 'Provide the Sizes from this list ["S", "XS", "M", "X", "L", "XXL", "XL"]', });
            }
