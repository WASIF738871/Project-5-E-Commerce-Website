...
const createProducts = async (req, res) => {
    try {
        ...
        if (!isValidRequestBody(data)) {
            return res.status(400).send({ status: false, message: "Provide the data for creating product " })
        }
        ...
        if (checkTitle) { return res.status(400).send({ status: false, message: "Product with this title is already present" }) }
        ...
        if (currencyFormat != "₹") {
            return res.status(400).send({ status: false, message: "currencySymbol should be in ₹" })
        }
        ...
        return res.status(201).send({ status: true, message: "Product is Created Successfully", data: createdProduct })
...
const updateProduct = async function (req, res) {
    try {
        ...
        //body empty
        if (!data || Object.keys(data).length == 0 && !files) return res.status(400).send({ status: false, message: "Provide Data in Body" })
        ...
        if (!productData) return res.status(404).send({ message: "Product is not present" })
        ...
        if (title != null) {
            ...
            if (titleData) return res.status(404).send({ message: `${title} is already present` })
            newObj['title'] = title
        }
        //description
        if (description!=null) {
        ...
        if (installments!=null) {
            ...
            if (!(!isNaN(Number(installments)))) return res.status(400).send({ status: false, message: "please enter valid installments and Should be in Number" }); 
        ...
        if (availableSizes!=null) {
            ...
            for (const element of subtrim) {
                ...
                if (Size.includes(element) === false) return res.status(400).send({ status: false, message: 'Provide the Sizes from this list ["S", "XS", "M", "X", "L", "XXL", "XL"]' })

            }
            let arr = productData.availableSizes
            for (let i = 0; i < arr.length; i++) {
                if (arr.includes(size)) {
                    return res.status(404).send({ status: false, message: `this ${size} size is already present ` })
                }
            }
        }
