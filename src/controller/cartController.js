            ...
            if(cartExist){
                ...
                for(let i=0; i<cartExist.items.length; i++){
                    if(cartExist.items[i].productId==productId && cartExist.items[i].quantity>0){
                        ...
                        return res.status(200).send({status:true,message:"Updated Your Cart ",data:result})
                    }
                }
                if(flag==0){
                    return res.status(404).send({status:false,message:`There is no Product with this ${productId} exist in ur cart`})
                }
            }else{
